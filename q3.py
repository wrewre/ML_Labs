...
import numpy as np  
# the function regression_test_and_train trains a regression model and compares the prediction on the test data and compares the metric values between train and test set.
def regression_test_and_train():  
    df = pd.read_csv(r"C:\Users\prana\OneDrive\Desktop\machine_learning\lab5\DCT_mal.csv")  
    X = df.drop(df.columns[2], axis=1).values  # extracts all attributes except target, as 2-d array
    ...
    rmse_test = np.sqrt(mse_test)  # RMSE test
    mape_test = np.mean(np.abs((y_test - y_test_pred) / y_test)) * 100  # MAPE test
